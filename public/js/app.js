...
(function(){ 
// cannot be .tooltip because it interferes w/ the tooltip overlay
$('.tooltip-trigger').tooltip({	// bootstrap tooltip not jQuery UI
	container: '.main-panel',
	placement: 'auto top'
});

// Project category detail show items
$(document).on('click', '.project-table .toggle-items', function() {
	$(this).closest('tr').next('tr.expense-items').slideToggle();
});

// Project category detail show all items
$(document).on('click', '.project-table .toggle-all-items', function() {
	var rows = $(this).closest('table').find('tr.expense-items');
...
$(document).on('click', '.advanced-search-toggle', function() {
	var advControls = $(this).closest('form').find('.advanced-controls');
	var isOpen = advControls.css('display') == 'none' ? false : true;
...
$(function() {
	showHideCategoryOptGroups();
});

$(document).on('change', '.search-controls select[name="project_id"]', showHideCategoryOptGroups);


function showHideCategoryOptGroups() {
	var projectSelect = $('.search-controls select[name="project_id"]');
	if (! projectSelect) return;

	var accountSelect = $('.search-controls select[name="account_id"]');
	var teamSelect = $('.search-controls select[name="team_id"]');

	var projectName = projectSelect.find('option:selected').text();
	var projectVal = projectSelect.val();

	accountSelect.find('optgroup').each(showHideGroups);
	teamSelect.find('optgroup').each(showHideGroups);
	...
	function showHideGroups() {
		...
		if ($(this).attr('label') == projectName) {
			$(this).show();
			return;
		}
...
// Toggle all expense items open/closed
$(document).on('click', '#show-hide-expense-items', function() {
	var table = $('#expense-table');
	var expenseItems = table.find('.expense-items');
	var expenseSummaries = table.find('.expense-summary');
	var button = $(this);
	
	if (table.hasClass('open')) {
		expenseItems.slideUp({
			complete: function() {
				button.html('show expense items');
				table.removeClass('open');
				expenseSummaries.addClass('closed');
			}
		});
		
	}
	else {
		expenseItems.slideDown({
			complete: function() {
				button.html('hide expense items');
			}
		});

		table.addClass('open');
		expenseSummaries.removeClass('closed');
	}
});

// toggle single expense item open/closed
$(document).on('click', '.expense-summary td .col:first-child', function() {
	var toggleRow = $(this).closest('tr');
	var expenseItems = toggleRow.next();

	if (expenseItems.css('display') == 'none') {
		expenseItems.slideDown();
		toggleRow.removeClass('closed');
	}
	else {
		expenseItems.slideUp({
			complete: function() {
				toggleRow.addClass('closed');
			}
		});
	}
});

// don't fire if link clicked
$(document).on('click', '.expense-summary td .col:first-child a', function(e) {
	e.stopPropagation();
	return true;
});
